...
def registred(message):
    try:
        ...
        db.new_user(message.from_user.id,'NULL',message.from_user.username, message.from_user.full_name, 'игрок со странным именем',1,0,0,str(datetime.datetime.now()).split()[0],0)
        user = db.read('users',message.from_user.id,'*') 
    return User(user)
